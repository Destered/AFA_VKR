...
class HtmlElementPreview(elements: List<HtmlElement>) : DialogWrapper(true) {
    ...
    override fun doOKAction() {
        // Предполагаем, что selectedElements - это список выбранных элементов
        val selectedElements = mutableListOf<HtmlElement>() // Заполните этот список
        // Получите объект Project каким-либо образом, например, из контекста или передайте в конструктор
        val project = ProjectManager.getInstance().defaultProject

        // Запускаем задачу
        ProgressManager.getInstance().run(GenerateTestsTask(project, selectedElements))
